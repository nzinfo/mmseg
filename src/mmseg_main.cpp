...
int segment(const char* file,Segmenter* seg, u1 bQuite = 0);
/*
Use this program 
Usage:
1)
...
int main(int argc, char **argv) {
	...
	u1 bQuite = 0;
	u1 bUcs2 = 0;
	while ((c = getopt(argc, argv, "t:b:u:d:o:rqU")) != -1) {
		switch (c) {
		case 'o':
			target_file = optarg;
			break;
...
int segment(const char* file,Segmenter* seg, u1 bQuite)
{
	std::istream *is;
